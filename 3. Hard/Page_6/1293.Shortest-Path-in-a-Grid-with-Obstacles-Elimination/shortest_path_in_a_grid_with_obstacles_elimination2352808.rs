// https://leetcode.com/problems/shortest-path-in-a-grid-with-obstacles-elimination/solutions/2352808/rust-bfs-with-optimization/
        ...
        while !dq.is_empty() {
            for _ in 0..dq.len() {
                ...
                for (&i, &j) in dir.iter().zip(dir.iter().skip(1)) {
                    ...
                    || new_col >= n || visited[new_row as usize][new_col as usize] >= left + 1 {
                        continue;
                    }
